...
协程从字面上来理解，就是可以实现协同的例程，也就是说，它可以用来实现多个例程之间的协同执行，比如使得例程A中的某个操作在例程B中的某个操作完成之后执行。
...
* 添加了throw()方法，可以在yield的位置抛入异常；异常如果没有处理，会传递回调用方
* 添加了close()方法，使得协程被提前终止，下次再调用send()方法会抛出StopIteration异常
  * colse()方式事实上是在yield的位置抛入了一个GeneratorExit异常
  * 到GeneratorExit异常的生成器一定不能产出新值，否则会触发RuntimeError
  * 该异常如果不处理，或者处理了之后重新抛出了StopIeration异常，异常不会向上传递
  * 抛出的其它异常会向上传递
* 允许返回值，但是是通过在StopIteration中设置value属性的方式，也就是说要通过捕获异常，然后从异常中获取value属性的方式来得到返回值

其中最重要的改变是添加了send()方法，使得从原来只能提供迭代输出的生成器，允许接受输入，在协程和协程的调用者之间实现了双向的输入输出，以此实现两者之间的有意义的协同。
...
作为委托生成的功能，yield from后面可以跟上一个迭代器，或者是另一个是使用了yield或者yield from语法的简单协程。

调用者调用委托生成器的方法，委托生成器将调用者的调用通过yield from委托给子生成器处理。因此yield from的作用，就好比是在调用者和子生成器之间架起了一条透明的沟通的管道。委托生成需要有一个终点，这个终点要么是一个生成器，要么是一个只是用了yield语法的简单协程。*（委托生成的实现原理：在委托生成器中通过yield获取调用者的输入，然后在子生成器上调用send()传入输入，获取子生成器的输出之后再通过yield向调用者返回输出。）*
...
* 子生成器如果没有处理异常，异常会在嵌套的生成器中逐层冒泡向上传递；如果没有任何生成器处理了这个异常，会一直传递回调用者
...
协程可以被加入事件循环，包装成一个task，task是future的子类。包装成task的协程事实上已经加入了事件循环的调度，将被异步地执行。下面是一个实例：
...
* 语句9会自动开启一个一个事件循环，将main()协程加入调度，并在协程结束之后自动关闭事件循环
...
* 语句5等待task1完成，因为这时候task1尚在事件循环的等待列表中，所以yield from会直接交出执行权，main()函数直接在yield from处返回
* 执行权回到事件循环，事件循环从等待列表中取出等待的任务task1开始执行
* 进入函数a，执行print('a start')，执行到语句1的时候，发起一个异步调用，休眠1秒钟，yield from交出执行权，从函数a中返回
* 执行权回到事件循环，事件循环从等待列表中取出等待的任务task2开始执行
* 进入函数a，执行print('b start')，执行到语句2的时候，发起一个异步调用，休眠1秒钟，yield from交出执行权，从函数b中返回
* 执行权回到事件循环，此时没有就绪的任务
...
* 事件循环取出就绪的main()协程，在语句5上恢复执行；执行到语句6，因为这时候task2已经完成，所以直接往下执行
* main协程完成，语句9返回
* 所以最终的输出顺序是
  * a start
  * b start
...
真正发起异步调用的是语句3、4，而语句5、6处的yield from实际上提供了一种类似“同步阻塞”的效果。注意这并不是真正的线程阻塞，只是在效果上“阻塞”了当前的协程，而不会阻塞到线程。

注意协程也可以直接被放到yield from语句之后，像语句7、8那样，这种情况下协程会被自动打包成一个任务。但是这时的输出会变成如下的顺序：
...
async/await版本使原生协程，不再基于生成器，也就是说没有实现\_\_iter\_\_和_\_next\_\_方法。
...
协程因为是单线程的，所以在任何一个环节都不能使用阻塞的调用，否则会阻塞整个线程。

如果实在是没有可用的非阻塞函数，只能使用阻塞函数，这时候可以通过loop提供的run_in_executor(executor, func, *args)函数将阻塞操作加入线程池执行。其中executor必须是一个concurrent.futures.Executor实例，如果传入None，会使用默认的executor。实例如下：
...
当需要动态地往event loop里加入协程的时候，可以在主线程中生成一个event loop，然后在一个新的线程中用loop.run_forever()启动event loop，当需要动态加入携程时，可以在主线程中通过调用asyncio.run_coroutine_threadsafe(coro, loop)将协程跨线程加入到event loop中。
...
线程的调度是抢占式，中断的时机不可控，而协程的调度是可控的，总是在可以预见的地方中断，所以在操作共享资源的时候对锁的依赖更小。